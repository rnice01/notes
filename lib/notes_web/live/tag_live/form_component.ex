defmodule NotesWeb.TagLive.FormComponent do
  ...
  def update(%{tag: tag} = assigns, socket) do
    changeset = Workspace.change_tag(tag)
  ...
  def handle_event("validate", %{"tag" => tag_params}, socket) do
    changeset =
      socket.assigns.tag
      |> Workspace.change_tag(tag_params)
  ...
  defp save_tag(socket, :edit, tag_params) do
    case Workspace.update_tag(socket.assigns.tag, tag_params) do
      {:ok, tag} ->
  ...
  defp save_tag(socket, :new, tag_params) do
    case Workspace.create_tag(tag_params) do
      {:ok, tag} ->
        notify_parent({:saved, tag})
